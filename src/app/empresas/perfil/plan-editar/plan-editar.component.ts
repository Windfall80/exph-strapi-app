...
export class PlanEditarComponent implements OnInit {
  ...
  cancelSubscription(){
    this.alert.open({
      ...
      text: `Tu suscripción se cancelara al final de el periodo actual y permanecerá activa hasta el día ${ moment(this.currentUser.company?.current_period_end).format('DD/MM/YYYY HH:mm') }.`, // Estos expirarán después de 30 días de la compra.
      ...
      if(result) {
        ...
        this.http.post<any>(`${environment.apiUrl}/companies/cancel-subscription`, {}).subscribe(
          (response: any)=>{
            ...
            this.alert.open({
              title: "Suscripción actualizada",
              text: `Tu suscripción esta activa hasta el día ${ moment(response.company?.current_period_end).format('DD/MM/YYYY HH:mm') }.`,
              showCancelButton: false
            }).then(()=>{
              this.router.navigateByUrl("/empresas/perfil");
            });
